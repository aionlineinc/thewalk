...
export function MemorialHero({
  ...
  const muted = dark ? "text-white/65" : "text-earth-600";
  const heading = dark ? "text-white" : "text-earth-900";
  const profileBorder = dark ? "border-white/20" : "border-white";
  const profileBg = dark ? "bg-white/10" : "bg-earth-100";

  return (
    <div className={dark ? "" : "mb-8"}>
      {/* Banner image — only in light mode (dark mode uses it as the cinematic bg) */}
      {!dark && bannerUrl ? (
      ...
      >
        {profileUrl ? (
          <div
            className={
              !dark && bannerUrl
                ? "-mt-14 shrink-0 relative z-10 sm:-mt-16"
                : "shrink-0"
            }
          >
            <img
              ...
              width={160}
              height={160}
              className={`h-28 w-28 rounded-xl border-4 ${profileBorder} ${profileBg} object-cover shadow-lg sm:h-36 sm:w-36`}
              loading="eager"
              decoding="async"
            />
          </div>
        ...
        <div className="min-w-0 flex-1">
          ...
          <h1 className={`mt-1 text-3xl font-bold tracking-tight sm:text-4xl ${heading}`}>
            {displayName}
          </h1>
          {birth || death ? (
          ...
          {age !== null ? (
            <p className={`mt-0.5 text-sm ${muted}`}>Age: {age}</p>
          ) : null}
          <a
            href="#guestbook"
            className={`mt-3 inline-flex items-center gap-1.5 text-sm font-medium underline-offset-4 hover:underline ${dark ? "text-white/80 hover:text-white" : ""}`}
            style={{ color: dark ? undefined : primaryColor || "#8c7b6d" }}
          >
