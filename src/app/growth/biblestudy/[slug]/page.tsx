...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = params;
  const article =
    (await getGrowthArticleByCategoryAndSlug("bible-study", slug)) ??
    GROWTH_ARTICLES.find((a) => a.slug === slug && a.category === "bible-study");
  if (!article) return { title: "Bible Study | theWalk Ministries" };
  return { title: `${article.title} | theWalk Ministries`, description: article.excerpt };
}

export default async function GrowthBibleStudyPage({ params }: Props) {
  const { slug } = params;
  const article =
    (await getGrowthArticleByCategoryAndSlug("bible-study", slug)) ??
    GROWTH_ARTICLES.find((a) => a.slug === slug && a.category === "bible-study") ??
